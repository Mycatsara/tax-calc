...
- **이미지(2026-09-02~, 글마다 2장)**: 원고의 `![alt](/img/파일.webp)` 줄은 그 자리에 `<figure class="fig"><img src="/img/파일.webp" width="1200" height="686" alt="…" loading="lazy" decoding="async"></figure>`로 변환한다. 규칙:
  ...
  - **프롬프트는 `Documents/기록/이미지생성_진행상황.md`의 "★ 프롬프트 규칙"을 따른다** — 사람은 상반신 이상 보이게(손만 금지) / 이목구비 단순·성별·나이 특정 없음 / 서명·글자·숫자 없음(명세서·지폐도 숫자 없이) / 물건 목록 명시 후 "그 외 물건 없음" / 소품 3개 이하. 생성 후 검수(축소 시트 1장을 보고 의심 부위만 확대·체크리스트 — 9/8 토큰 절감, 원본 전부를 그대로 보지 않는다) 통과 전 게시 금지
...
## 0단계: 글감 고르기 (9/6~ radar)

- 새 글 주제는 **radar**에서 고른다: `radar.cmd` 실행 → 일간 탭 → **보기: taxtool** → ▶ 분석 → 월검색 큰 순서로 보되 "AI가 답하나?"가 ❌(도구필요·최신데이터·경험필요)인 것, 경쟁 낮음/중간, 발행됨 아닌 것을 우선. 시즌 글은 "월별 시즌 달력" 탭에서 다음 달 피크를 본다
- 후보 3개(키워드·월검색·경쟁·왜 지금)를 운영자에게 → 승인된 것은 상세 창에서 **배정 taxtool + 상태 채택** → "📋 원고 프롬프트 복사"를 원고의 출발점으로. 연관 키워드 10개는 소제목·본문 어휘에 녹인다
- 예외: 계산기 위성글처럼 계획된 글은 radar 없이 가되, 제목 키워드의 검색량은 radar 검색창으로 한 번 확인한다
